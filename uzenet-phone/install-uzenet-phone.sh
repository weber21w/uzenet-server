...
SERVICE_NAME="uzenet-phone"
EXEC_NAME="uzenet-phone-server"
SERVICE_USER="uzenet"
SERVICE_ROOT="/var/lib/uzenet-phone"
...
install -d -o "${SERVICE_USER}" -g "${SERVICE_USER}" -m 755 "${SERVICE_ROOT}"

echo "[5/8] Writing systemd service file to ${SERVICE_FILE}..."
cat > "${SERVICE_FILE}" <<EOF
[Unit]
Description=Uzenet Phone Gateway
After=network.target

[Service]
Type=simple
ExecStart=${INSTALL_PATH} --log-level 4
...
echo "Installed and started ${SERVICE_NAME}. Log file: ${LOG_FILE}"
