...
SERVICE_NAME=uzenet_phone           # underscore for binary name pattern
LOG_FILE=/var/log/uzenet-phone.log
BINARY=/usr/local/bin/uzenet-phone-server
SERVICE_FILE=/etc/systemd/system/uzenet-phone.service
DEFAULT_PORT=57430
...
if [[ "${1:-}" == "--json" ]]; then
    ...
    jq -n \
        --arg service "$(systemctl is-active uzenet-phone)" \
        --arg enabled "$(systemctl is-enabled uzenet-phone 2>/dev/null)" \
...
echo "Uzenet Phone Server Status"
echo "=========================="

echo -n "Service: "; systemctl is-active --quiet uzenet-phone && echo "RUNNING" || echo "STOPPED"
echo -n "Enabled at boot: "; systemctl is-enabled uzenet-phone 2>/dev/null || echo "No"
...
systemctl status uzenet-phone --no-pager --lines=5 || echo "(service not found)"
